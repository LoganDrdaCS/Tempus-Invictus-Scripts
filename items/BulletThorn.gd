...
export var amplitude : float =  4.8
export var frequency : float = 2.9
var time : float = 0.0
...
func _on_BulletThorn_body_entered(body: Node) -> void:
	match body.get_collision_layer():
		2:
			BusForSignals.emit_signal("bullet_thorn_obtained")
			obtained_by_player()
